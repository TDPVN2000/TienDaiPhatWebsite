import { useQuery } from "@tanstack/react-query";
import { Pagination } from "antd";
import { getProjectsApi } from "api/projects";
import HeaderContent from "components/HeaderContent";
import Loading from "components/Loading";
import { QueryKey } from "constants/enums";
import { useState } from "react";
import { useTranslation } from "react-i18next";
import { useNavigate } from "react-router-dom";
import TableProjects from "./components/TableProjects";
import styles from "./styles.module.scss";

export default function ProjectsManagement() {
  const [t] = useTranslation();
  useState<boolean>(false);
  const navigate = useNavigate();

  const [currentPage, setCurrentPage] = useState(1);
  const pageSize = 9;

  const {
    data: projectData = [],
    isFetching,
    refetch,
  } = useQuery({
    queryKey: [QueryKey.LIST_PROJECT],
    queryFn: () => getProjectsApi(),
  });

  const startIndex = (currentPage - 1) * pageSize;
  const endIndex = currentPage * pageSize;
  const visibleProjects = projectData.slice(startIndex, endIndex);

  const handleChange = (page: number) => {
    setCurrentPage(page);
  };

  return (
    <div className={styles.container}>
      {isFetching && <Loading />}
      <HeaderContent
        title={t("Danh sách dự án")}
        buttonAdd={{
          title: t("Tạo thông tin dự án"),
          onClick: () => navigate("/projects-management/store"),
        }}
      />
      <div className={styles.body}>
        <div className={styles.viewFilter}>
          <div className={styles.viewTotalNotifications}>
            <span className={styles.totalNotifications}>
              Dự án đã hoàn thành: {projectData?.length}
            </span>
          </div>
        </div>
        <div className="content-page">
          <TableProjects
            data={visibleProjects || []}
            isLoading={false}
            onRefetch={refetch}
          />

          <Pagination
            current={currentPage}
            pageSize={pageSize}
            total={projectData.length}
            onChange={handleChange}
            className={styles.pagination}
            showSizeChanger={false}
          />
        </div>
      </div>
    </div>
  );
}
